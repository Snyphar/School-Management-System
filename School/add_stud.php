		...
		<?php include"navbar.php";?><br>
			...
			<div id="section">
	              ...
	              if($re->num_rows>0)
								{
									$row1=$re->fetch_assoc();
									$cl_id = $row1["CID"];
								$sq="insert into student(RNO,NAME,FNAME,DOB,GEN,PHO,MAIL,ADDR,SCLASS,SSEC,SIMG,AID,PASS,CID) values('{$_POST["rno"]}','{$_POST["name"]}','{$_POST["fname"]}','{$edate}','{$_POST["gen"]}','{$_POST["pho"]}','{$_POST["email"]}','{$_POST["addr"]}','{$_POST["cla"]}','{$_POST["sec"]}','{$target_file}','{$_SESSION["AID"]}','{$_POST["pass"]}','{$cl_id}')";
				...
				<label> Parent's Mail Id</label><br>
					...
					<textarea rows="3" name="pass"></textarea><br><br>
